...
#define CHECK_SIN_RANGE(VAR) if (!check_sin_range(VAR, #VAR)) return;
#define CHECK_COS_RANGE(VAR) if (!check_cos_range(VAR, #VAR)) return;
...
public:
	...
	Integral(ForwardRayTracing<Real, Complex>& data) : data(data) {}
	...
	void check_cos_range(const Real& cos_x, const char* name) {
		if (cos_x < -1 || cos_x > 1) {
